...
- Do not start a dev server for routine validation unless the user explicitly asks for one; build the release desktop executable instead when a runnable app artifact is needed.
...
- Run `bun run tauri:build` to produce a release desktop executable; prefer this over starting a dev server or fast debug build.
